    ...
    instance{_instance}
{
    ...
    _random_offset_lat = 0.0;
    _random_offset_lon = 0.0;
    _distance_tracking_initialized = false;
}
...
void GPS::update()
{
    ...
    // fill in accuracies
    d.horizontal_acc = 50.0f;  // Set horizontal accuracy to 50m
    ...
    // Distance tracking and random offset logic
    if (!_distance_tracking_initialized) {
        _last_lat = d.latitude;
        _last_lon = d.longitude;
        _distance_tracking_initialized = true;
    } else {
        // Calculate distance traveled since last update using Haversine formula
        double lat1_rad = radians(_last_lat);
        double lon1_rad = radians(_last_lon);
        double lat2_rad = radians(d.latitude);
        double lon2_rad = radians(d.longitude);
        
        double dlat = lat2_rad - lat1_rad;
        double dlon = lon2_rad - lon1_rad;
        
        double a = sinf(dlat/2) * sinf(dlat/2) + cosf(lat1_rad) * cosf(lat2_rad) * sinf(dlon/2) * sinf(dlon/2);
        double c = 2 * atan2f(sqrtf(a), sqrtf(1-a));
        double distance_m = 6371000 * c;  // Earth radius in meters
        
        _cumulative_distance += distance_m;
        
        // Check if we've traveled 500m, if so generate new random offset
        if (_cumulative_distance >= 500.0) {
            // Store current GPS position (including previous offset) as the reference point
            double reference_lat = d.latitude + _random_offset_lat;
            double reference_lon = d.longitude + _random_offset_lon;
            
            // Generate random direction (0-360 degrees)
            float random_angle = (get_random16() % 36000) / 100.0f;  // 0.01 degree precision
            float angle_rad = radians(random_angle);
            
            // Calculate new position 200m away from reference position in the random direction
            double offset_distance_m = 200.0;
            double earth_rad_inv = 1.569612305760477e-7;
            double lng_scale_factor = earth_rad_inv / cos(radians(reference_lat));
            
            double new_lat = reference_lat + degrees(offset_distance_m * cosf(angle_rad) * earth_rad_inv);
            double new_lon = reference_lon + degrees(offset_distance_m * sinf(angle_rad) * lng_scale_factor);
            
            // Calculate the offset needed to reach the new position from the original position
            _random_offset_lat = new_lat - d.latitude;
            _random_offset_lon = new_lon - d.longitude;
    ...
    // Apply current random offset
    d.latitude += _random_offset_lat;
    d.longitude += _random_offset_lon;
