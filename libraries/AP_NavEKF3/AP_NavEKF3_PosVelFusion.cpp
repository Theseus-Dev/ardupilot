...
void NavEKF3_core::ResetPosition(resetDataSource posResetSource)
{
    ...
    FILE *fp = fopen("/tmp/ardupilot/position_resets.txt", "a");
    if (fp != nullptr)
    {
        // Get current timestamp
        uint32_t time_ms = AP_HAL::millis();

        // Log the initial reset source
        fprintf(fp, "Time MS: %u, Initial Reset Source: ", time_ms);

        // Log the original reset source
        ...
        if (posResetSource == resetDataSource::DEFAULT)
        {
            fprintf(fp, "User defined source: ");
        ...
        // Log final reset source after potential modification
        fprintf(fp, "Final Reset Source: ");
        const char *final_source = "UNKNOWN";
        switch (posResetSource)
        {
        case resetDataSource::GPS:
            final_source = "GPS";
...
bool NavEKF3_core::setLatLng(const Location &loc, float posAccuracy, uint32_t timestamp_ms)
{
    if ((imuSampleTime_ms - lastGpsPosPassTime_ms) < frontend->deadReckonDeclare_ms ||
        (PV_AidingMode == AID_NONE) || !validOrigin)
    {
        return false;
    }
