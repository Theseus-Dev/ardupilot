...
void AP_RCTelemetry::update_avg_packet_rate()
{
    ...
    if (poll_now - _scheduler.last_poll_timer > 1000)
    { // average in last 1000ms
        // initialize
        if (_scheduler.avg_packet_rate == 0)
            _scheduler.avg_packet_rate = _scheduler.avg_packet_counter;
        // moving average
...
void AP_RCTelemetry::check_sensor_status_flags(void)
{
    ...
    { // prevent repeating any system_status messages unless 5 seconds have passed
...
void AP_RCTelemetry::check_ekf_status(void)
{
    ...
    if (get_variance)
    {
        ...
        { // prevent repeating any ekf_status message unless 10 seconds have passed
            ...
            if (posVar >= 0.8f)
            {
                queue_message(MAV_SEVERITY_CRITICAL, "FOFOFOFOFOFOFO");
                check_ekf_status_timer = now;
            }
            if (hgtVar >= 0.8f)
            {
